...
export class PatientProfileComponent implements OnInit {
  ...
  saveProfile() {
    // Validation
    if (!this.profileData.name || !this.profileData.email || !this.profileData.phone) {
      this.toastr.error('Name, email, and phone are required');
      return;
    }

    this.saving = true;

    this.api.updatePatientProfile(this.profileData)
      .then((res: any) => {
        ...
        if (res.data.status) {
          ...
          if (patientData) {
            ...
            localStorage.setItem('patient', JSON.stringify(parsed));
          }
        } else {
